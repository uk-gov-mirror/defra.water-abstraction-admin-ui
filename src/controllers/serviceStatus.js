...
async function serviceStatus(request, reply) {
  ...
  try {
    const verificationData = await CRM.verifications.findMany({}, {}, {
      "perPage": 1
    })
  ...
  try {
    const importDataComplate = await water.pending_import.findMany({
      "status": 1
    }, {}, {
      "perPage": 1
    })
  ...
  try {
    const importDataPending = await water.pending_import.findMany({
      "status": 0
    }, {}, {
      "perPage": 1
    })
